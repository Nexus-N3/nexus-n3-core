...
class Client:
    ...
    def handle_event(self, event: dict):
        ...
        if evt_type == mt.EVT_SERVER_READY:
            self.send_command({
                ...
                "payload": {"subjects": self.subjects, "init_label": "Summary_test"},
            })
            ...
            now = time.time()
            if self._last_result_ts is None:
                delta = None
            else:
                delta = now - self._last_result_ts
    ...
    def _handle_stream_sequence(self):
        ...
        self.send_command({"type": mt.CMD_START_STREAM_FOR_ALL, "payload": {"tag": "summary_test"}})
            ...
            "sensors": [
                {
                    ...
                    "number_of": 1,
                    "compute_algorithm": {"name": "generic_data_summary", "inputs": {}},
                    "attributes": {"STREAMS": ["ECG", "HR"]},
                    "locations": ["CHEST"],
                },
                {
