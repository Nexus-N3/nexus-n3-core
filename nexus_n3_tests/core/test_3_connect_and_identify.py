...
class Client:
    ...
    def handle_event(self, event: dict):
        ...
        # --- SYSTEM INITIALIZED → DISCOVER SENSORS ---
        ...
        # --- SENSORS DISCOVERED → CONNECT ---
        elif evt_type == mt.EVT_SENSORS_DISCOVERED:
            if not payload:
                return
            print(f"discovered devices {payload}")
        ...
        # --- SENSOR CONNECTED → IDENTIFY / DISCONNECT ---
        elif evt_type == mt.EVT_SENSOR_CONNECTED:
            # Trigger identify on all body locations
            for sub in self.subjects:
            ...
            # Optional delay before disconnect
            time.sleep(5)
