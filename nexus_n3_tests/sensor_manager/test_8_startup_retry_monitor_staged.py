...
class StartupRetryPacketMonitorTest:
    ...
    def __init__(
        ...
        hci_adapter: str,
        staged_connect: bool,
        staged_connect_first_group_size: int,
        staged_connect_gap_seconds: float,
    ):
        ...
        self.staged_connect = staged_connect
        self.staged_connect_first_group_size = staged_connect_first_group_size
        self.staged_connect_gap_seconds = staged_connect_gap_seconds
        self.staged_connect_started = False
        self.staged_second_group_started = False
        self.staged_first_group_addresses: list[str] = []
        self.staged_second_group_addresses: list[str] = []
    ...
    def _reset_attempt_state(self, attempt_number: int):
        ...
        self.staged_connect_started = False
        self.staged_second_group_started = False
        self.staged_first_group_addresses = []
        self.staged_second_group_addresses = []
    ...
    def on_discover(self, payload):
        ...
        discovered_addresses = discovered_addresses[: self.sensor_count]

        if len(discovered_addresses) < self.sensor_count:
            self.attempt_reason = (
                f"Discovery found only {len(discovered_addresses)} of "
        ...
        first_size = min(
            self.staged_connect_first_group_size,
            len(discovered_addresses),
        )
        self.staged_first_group_addresses = discovered_addresses[:first_size]
        self.staged_second_group_addresses = discovered_addresses[first_size:]
        self.staged_connect_started = True

        print("STAGED CONNECT group 1:", self.staged_first_group_addresses)
        self.manager.connect_specific_sensors(self.staged_first_group_addresses)
    ...
    def on_connected(self, payload):
        ...
        with self._lock:
            ...
            if (
                self.staged_connect
                and self.staged_connect_started
                and not self.staged_second_group_started
                and len(self.connected_addresses) >= len(self.staged_first_group_addresses)
                and self.staged_second_group_addresses
            ):
                self.staged_second_group_started = True
                action = "schedule_second_group"
            elif len(self.connected_addresses) < self.sensor_count:
                self.attempt_outcome = "retry"
                self.attempt_reason = (
        ...
        if action == "schedule_second_group":
            print(
                "STAGED CONNECT group 1 connected. "
                f"Waiting {self.staged_connect_gap_seconds:.1f}s before group 2."
            )
            threading.Thread(target=self._connect_staged_second_group, daemon=True).start()
    ...
    def _connect_staged_second_group(self):
        time.sleep(self.staged_connect_gap_seconds)
        with self._lock:
            if self.manager is None:
                return
            if self.attempt_outcome in {"retry", "fatal"}:
                return
            addresses = list(self.staged_second_group_addresses)

        print("STAGED CONNECT group 2:", addresses)
...
def parse_args():
    ...
    parser.add_argument(
        "--staged-connect",
        action="store_true",
        help="Discover all sensors, then connect them in two staged groups.",
    )
    parser.add_argument(
        "--staged-connect-first-group-size",
        type=int,
        default=3,
        help="Number of sensors to connect in the first staged group.",
    )
    parser.add_argument(
        "--staged-connect-gap-seconds",
        type=float,
        default=5.0,
        help="Delay between staged connection groups.",
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    exit_code = StartupRetryPacketMonitorTest(
        ...
        staged_connect=args.staged_connect,
        staged_connect_first_group_size=args.staged_connect_first_group_size,
        staged_connect_gap_seconds=args.staged_connect_gap_seconds,
    ).run()
