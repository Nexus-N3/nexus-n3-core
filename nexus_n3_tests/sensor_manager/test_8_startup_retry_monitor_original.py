...
class StartupRetryPacketMonitorTest:
    """Retry startup until all sensors stabilize, then run a 30-second capture."""
    ...
    def __init__(self, sensor_count: int, stream_seconds: int, timeout_seconds: int):
        ...
        self.max_start_attempts = 5
        self.startup_stability_window_seconds = 5.0
        self.retry_delay_seconds = 2.0
        self.identify_pause_seconds = 1.0
        self.startup_packets_required = 20
        self.startup_min_rate_hz = 50.0
        self.startup_min_observation_seconds = 1.5
    ...
    def _begin_retry_cleanup(self, reason: str):
        action = None
        with self._lock:
            if self.attempt_outcome in {"retry", "fatal", "success"} and self.awaiting_disconnect:
                return
            self.attempt_outcome = "retry"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "30-second packet monitor capture."
        )
    )
    parser.add_argument(
        "--sensor-count",
