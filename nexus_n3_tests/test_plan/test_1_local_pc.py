...
class Client:
    ...
    def handle_event(self, event: dict):
        ...
        if evt_type == mt.EVT_SERVER_READY:
            ...
            # Identify is fire-and-forget for this sensor flow.
            # Start stream sequence after an identify settle delay.
            ...
            if self.stop_stage == "stream" and not self.stream_sequence_started:
                self.stream_sequence_started = True
                threading.Thread(target=self._handle_stream_sequence, daemon=True).start()
    ...
    def _handle_stream_sequence(self):
        """Run identify + start streaming with delays in a thread."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Nexus N3 Core local PC staged test plan")
